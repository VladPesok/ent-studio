...
const DiagnosesTable: React.FC<DiagnosesTableProps> = ({ data, loading, onDataChange }) => {
  ...
  useEffect(() => {
    const loadSavedState = async () => {
      const savedState = await loadTableState(TABLE_KEYS.DIAGNOSES);
      if (savedState) {
  ...
  const handleTableChange = (
    ...
    saveTableState(TABLE_KEYS.DIAGNOSES, stateToSave);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await window.ipcRenderer.invoke('db:dict:diagnoses:update', editingItem.id, editName.trim());
  ...
  const handleDelete = async (item: DictionaryItem) => {
    try {
      await window.ipcRenderer.invoke('db:dict:diagnoses:delete', item.id);
  ...
  const handleRestore = async (item: DictionaryItem) => {
    try {
      await window.ipcRenderer.invoke('db:dict:diagnoses:restore', item.id);
  ...
  return (
    <>
      ...
      <Modal
        title={t('dictionaries.modals.editDiagnosis')}
      ...
      >
        <Input
          ...
          placeholder={t('dictionaries.placeholders.enterDiagnosisName')}
